...
# WeatherAPI ключ. Можно переопределить через переменную окружения WEATHER_API_KEY.
...
if not MONGO_URL:
    raise SystemExit(
        "❌ Переменная окружения MONGO_URL не задана. "
        "Укажите строку подключения к MongoDB Atlas в настройках хостинга."
    )
if not DEVELOPER_ID:
...
# Файлы для хранения данных (users хранятся в MongoDB, остальные — JSON)
...
# Этот файл нужен ОТДЕЛЬНО от пользовательских данных (Mongo),
# потому что подтверждение может прийти ДО создания аккаунта
# (когда user-объекта ещё нет).
...
# Состояние пакетного ввода учителей: одно сообщение, несколько строк
# «Предмет: Учитель» — за один заход добавляется сразу всё.
BATCH_ADD_TEACHERS = 106
...
_SANITIZE_TARGET_FILES = [
    "classes.json", "timers.json", "anonymous_messages.json",
...
# ==================================
# === HTTP-СЕРВЕР (Render требует слушать порт)               ===
# ==================================
# Render помечает сервис как "упавший", если процесс не слушает HTTP-порт,
# поэтому даже бот-поллер должен поднимать минимальный веб-сервер.

async def health_check(request):
    """GET /  и  GET /health  — отвечает 'OK' для health-check Render."""
    return web.Response(text="OK", status=200)


async def run_http_server():
    """Запускает aiohttp веб-сервер на 0.0.0.0:$PORT (по умолчанию 10000)
    и держит его живым до отмены задачи. Используется в asyncio.gather
    параллельно с поллингом бота."""
    app = web.Application()
    ...
    try:
        # Сервер живёт до тех пор, пока корутину не отменят (SIGTERM/Ctrl+C).
...
# === MongoDB Atlas (motor) ===
# ==================================
# Полностью заменяет хранение пользователей в users.json. Документы
# в коллекции `users` имеют форму:
#   { "user_id": "<telegram-id>", ...поля User.to_dict()... }
# и индексируются по полю user_id (уникально).

_mongo_client: AsyncIOMotorClient | None = None
_mongo_db = None
_users_collection = None
# Имя БД по умолчанию, если строка подключения не содержит явного имени.
_MONGO_DEFAULT_DB_NAME = _env("MONGO_DB_NAME", "school_bot")


async def init_mongo():
    """Инициализирует Motor-клиент и создаёт уникальный индекс по user_id.

    Должно вызываться один раз при старте, до того как загружается кэш
    пользователей и до того как обработчики начнут читать данные."""
    global _mongo_client, _mongo_db, _users_collection
    _mongo_client = AsyncIOMotorClient(
        MONGO_URL,
        serverSelectionTimeoutMS=10000,
        # Имя приложения помогает отслеживать соединения в Atlas.
        appname="school-tg-bot",
    )
    # Если в URL не указано имя БД, motor вернёт None — fallback на дефолт.
    try:
        default_db = _mongo_client.get_default_database()
    except Exception:
        default_db = None
    if default_db is None:
        _mongo_db = _mongo_client[_MONGO_DEFAULT_DB_NAME]
    else:
        _mongo_db = default_db
    _users_collection = _mongo_db["users"]

    # Гарантируем уникальный индекс по user_id (строкой, как Telegram id).
    await _users_collection.create_index("user_id", unique=True)

    # Лёгкий ping, чтобы убедиться, что соединение работает.
    await _mongo_db.command("ping")
    logger.info(
        f"MongoDB подключена: db={_mongo_db.name}, collection=users"
    )


async def close_mongo():
    """Корректно закрывает соединение с MongoDB при остановке бота."""
...
async def get_user_from_db(user_id):
    """Ищет пользователя в коллекции users по user_id. Возвращает dict или None."""
    if _users_collection is None:
        return None
    doc = await _users_collection.find_one({"user_id": str(user_id)})
    if doc is not None:
        # _id (ObjectId) неудобно сериализовать и не нужен бизнес-логике.
        doc.pop("_id", None)
    return doc


async def update_user_in_db(user_id, data):
    """Upsert документа пользователя в коллекции users по user_id.

    Перезаписывает все указанные поля; user_id всегда нормализуется к строке."""
    if _users_collection is None:
        return
...
async def delete_user_from_db(user_id):
    """Удаляет документ пользователя по user_id. Возвращает True/False."""
    if _users_collection is None:
        return False
    result = await _users_collection.delete_one({"user_id": str(user_id)})
    return result.deleted_count > 0


def _schedule_mongo_task(coro):
    """Из синхронного кода планирует асинхронную задачу в текущем event loop.

    Используется обёртками save_user / save_users / update_user, чтобы
    не блокировать обработчики на сетевом IO. Если активного цикла нет —
    выполняем корутину синхронно через asyncio.run (только при тестах/CLI)."""
    try:
    ...
    # Нет активного цикла — выполняем сразу. На проде это не должно случаться,
    # но при импорте/тестах удобно, чтобы запись всё равно произошла.
...
def save_data(filename, data):
    try:
        # Синхронное сохранение для надежности
...
def load_prices():
    ...
    return load_data(PRICES_FILE, default_prices)
...
def timeout(seconds):
    ...
    Теперь это no-op по части тайм-аутов: аргумент seconds принимается для
    обратной совместимости, но сам декоратор не завершает разговор и не
    глотает исключения. Дополнительно: если пользователь, находясь в
    каком-то FSM-шаге, нажал reply-кнопку из главного меню, обёртка
    распознаёт это и переадресовывает запрос в `handle_main_menu`, чтобы
    кнопка сработала, а пользователь не «застрял» в шаге диалога с
    ошибкой «нажмите ещё раз».
    """
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.warning(f"menu-escape pre-check failed in {getattr(func, '__name__', '?')}: {e}")
            try:
...
class User:
    def __init__(self, user_id, username="", first_name=""):
        ...
        # Получает ли разработчик уведомления о новых пользователях. Поле имеет
        # смысл только для пользователя с user_id == DEVELOPER_ID, для остальных
        # просто хранится и игнорируется. По умолчанию — ВКЛ.
        self.notify_new_users = True
    ...
    def to_dict(self):
        return {
            ...
            'notify_new_users': getattr(self, 'notify_new_users', True),
        }
    ...
    def from_dict(cls, data):
        ...
        if not hasattr(user, 'notify_new_users'):
            user.notify_new_users = True
        return user
...
async def bulk_load_users_from_mongo():
    """Один раз при старте загружает всех пользователей из MongoDB в кэш.
    Дальше синхронный load_users() возвращает уже прогретый кэш без сетевых
    запросов — это сохраняет совместимость со всеми существующими хендлерами."""
    global _users_cache
    if _users_collection is None:
        logger.error("bulk_load_users_from_mongo: коллекция users не инициализирована")
        return
    users = {}
    ...
    async for doc in cursor:
        try:
            ...
            if not uid:
                continue
            users[uid] = User.from_dict(doc)
        except Exception as e:
            logger.error(f"Mongo: ошибка десериализации пользователя: {e}")
    _users_cache = users
    _cache_last_update['users'] = time.time()
    logger.info(f"Mongo: загружено пользователей в кэш: {len(users)}")


def load_users():
    """Возвращает прогретый из MongoDB in-memory кэш пользователей.

    Кэш заполняется в init-фазе через bulk_load_users_from_mongo() и
    обновляется in-place при save_user / save_users."""
    return _users_cache


def save_users(users):
    """Обновляет in-memory кэш и асинхронно зеркалит изменения в MongoDB.

    Формально перезаписывает в Mongo все переданные пользователи; для
    одиночных изменений эффективнее вызывать save_user(user)."""
    global _users_cache
    _users_cache = users
    for uid, user in users.items():
        try:
            _schedule_mongo_task(update_user_in_db(uid, user.to_dict()))
        except Exception as e:
            logger.error(f"save_users: не удалось запланировать запись {uid}: {e}")
    return True
...
def get_instructions():
    ...
    try:
        ...
        text = text.format(
            ...
            broadcast=prices.get('broadcast', 200),
        )
...
def block_user_in_class(user_id, class_code, admin_id):
    class_obj = get_class_by_code(class_code)
    if class_obj and str(user_id) not in class_obj.blocked_users:
...
def get_user(user_id):
    """Ищет пользователя по user_id (прогретый из MongoDB кэш).

    Совместимо со всеми существующими (синхронными) хендлерами. Прямой
    доступ в MongoDB — async-функция get_user_from_db(user_id)."""
    try:
        users = load_users()
...
def save_user(user):
    """Сохраняет одного пользователя: обновляет кэш и async-upsert в MongoDB."""
    global _users_cache
    if user is None:
        return False
    _users_cache[str(user.user_id)] = user
    try:
        _schedule_mongo_task(update_user_in_db(user.user_id, user.to_dict()))
    except Exception as e:
        logger.error(f"save_user: не удалось запланировать запись {user.user_id}: {e}")
    return True


def update_user(user_id, data):
    """Обновляет данные пользователя в коллекции users по user_id.

    Принимает dict с полями User.to_dict(); обновляет кэш и асинхронно
    делает upsert в MongoDB. Это публичное API хранилища users."""
    global _users_cache
    if not data:
        return False
    payload = dict(data)
    payload["user_id"] = str(user_id)
    # Обновляем кэш, чтобы синхронные читатели видели свежие данные сразу.
    try:
        existing = _users_cache.get(str(user_id))
        if existing is None:
            _users_cache[str(user_id)] = User.from_dict(payload)
        else:
            merged = existing.to_dict()
            merged.update(payload)
            _users_cache[str(user_id)] = User.from_dict(merged)
    except Exception as e:
        logger.error(f"update_user: не удалось обновить кэш {user_id}: {e}")
    try:
        _schedule_mongo_task(update_user_in_db(user_id, payload))
    except Exception as e:
        logger.error(f"update_user: не удалось запланировать запись {user_id}: {e}")
    return True
...
def get_holidays_count(class_obj):
...
def format_homework(homework_dict, date_str=None):
    ...
    for subject, assignments in homework_dict.items():
        ...
        for assignment in assignments:
            ...
            # Дата добавления — пользователь видит, когда задание задали.
            added_at = assignment.get('added_at')
            if added_at:
                # added_at хранится как «YYYY-MM-DD HH:MM», но человеку проще
                # читать «ДД.ММ.ГГГГ». Если формат другой — показываем как есть.
                pretty = added_at
                try:
                    dt = datetime.strptime(added_at, "%Y-%m-%d %H:%M")
                    pretty = dt.strftime("%d.%m.%Y")
                except Exception:
                    try:
                        dt = datetime.strptime(added_at, "%Y-%m-%d")
                        pretty = dt.strftime("%d.%m.%Y")
                    except Exception:
                        pass
                line += f"  _(задано: {pretty})_"
            text += line + "\n"
        text += "\n"
...
def get_teachers_edit_keyboard(class_obj):
    ...
    keyboard.append([InlineKeyboardButton("📥 Добавить всех сразу", callback_data="batch_add_teachers")])
...
def get_main_menu_keyboard(user):
    ...
    standard_buttons = [
        ...
        "📨 Мои анонимные сообщения", "🎓 Управление классами",
...
def get_developer_keyboard():
    # Состояние тоггла «уведомления о новых пользователях» подтягиваем
    # прямо из настроек разработчика, чтобы текст кнопки отражал
    # актуальное значение.
    dev_user = get_user(DEVELOPER_ID) if DEVELOPER_ID else None
    notify_new = getattr(dev_user, 'notify_new_users', True) if dev_user else True
    notify_label = (
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(notify_label, callback_data="dev_toggle_new_user_notif")],
        [InlineKeyboardButton("❌ Отмена", callback_data="cancel_action")]
    ]
    return InlineKeyboardMarkup(keyboard)
...
def get_anonymous_reply_keyboard(message_id):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💬 Ответить анонимно", callback_data=f"reply_anon_{message_id}")],
        [InlineKeyboardButton("❌ Отмена", callback_data="cancel_anon_reply")]
    ])
...
async def _ai_thinking_animation(message, stop_event):
    """Циклически редактирует сообщение «думает», добавляя точки/анимацию,
    пока не выставлен `stop_event`. Работает абсолютно безопасно: ловит
    любые ошибки правки (BadRequest при идентичном тексте, FloodWait и т. д.).
    """
    frames = [
        "🤖 DEVO+ai думает.",
        "🤖 DEVO+ai думает..",
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # Любая ошибка (одинаковый текст, ограничения API) — просто
            # ждём следующего кадра, не падаем.
            pass
        try:
            await asyncio.wait_for(stop_event.wait(), timeout=0.7)
...
async def handle_ai_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПУНКТ: индикатор «печатает…» не должен ронять хендлер, если сеть моргнула
    try:
    ...
    # Анимация «думает». Отправляем placeholder, пока ждём ответ.
    thinking_msg = None
    stop_event = asyncio.Event()
    anim_task = None
    try:
        thinking_msg = await update.message.reply_text("🤖 DEVO+ai думает.")
        anim_task = asyncio.create_task(_ai_thinking_animation(thinking_msg, stop_event))
    except Exception as e:
        logger.warning(f"thinking placeholder failed: {e}")
        thinking_msg = None
        anim_task = None
    ...
    async def _stop_anim():
        # Аккуратно останавливаем анимацию и убираем placeholder.
        if anim_task is not None:
            try:
                stop_event.set()
                await asyncio.wait_for(anim_task, timeout=2)
            except Exception:
                try:
                    anim_task.cancel()
                except Exception:
    ...
    try:
        ...
        try:
            await update.message.reply_text(
                "⏱ DEVO+ai не ответил вовремя. Попробуйте ещё раз — я всё ещё здесь."
            )
        except Exception:
            pass
        return
        ...
        try:
            await update.message.reply_text(
                "❌ Произошла ошибка при обработке AI запроса. Попробуйте позже."
            )
        except Exception:
            pass
        return

    # Останавливаем анимацию и удаляем placeholder ПЕРЕД отправкой ответа.
    await _stop_anim()
    ...
    # (раньше это вызывало BadRequest и могло выглядеть как «бот молчит»).
    prefix = "🤖 DEVO+ai:\n\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user:
        ...
        save_user(user)
        # Сообщаем разработчику о новом пользователе. Тоггл живёт в
        # настройках самого пользователя-разработчика (User.notify_new_users).
        try:
            await _notify_developer_about_new_user(context, user)
        except Exception as e:
            logger.error(f"notify dev about new user failed: {e}")
    ...
    if not user.birthday:
        ...
        await update.message.reply_text(
            "⏰ Введите текущее местное время в формате ЧЧ:ММ (например, 14:30):",
            reply_markup=get_cancel_keyboard()
        )
        return SET_TIME
    else:
...
async def disclaimer_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "accept_disclaimer":
        ...
        if not user.setup_completed:
            await query.edit_message_text("✅ Отлично!\n\n⏰ Теперь введите текущее местное время в формате ЧЧ:ММ (например, 14:30):")
            return SET_TIME
        else:
            await show_main_menu(update, context, user)
            return MAIN_MENU
...
async def instructions_read_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new_user:
        try:
            ...
            logger.error(f"notify dev about new user (instructions): {e}")
    ...
    if not user.birthday:
        ...
        await query.edit_message_text("⏰ Введите текущее местное время в формате ЧЧ:ММ (например, 14:30):")
        return SET_TIME
    else:
        await show_main_menu(update, context, user)
        return MAIN_MENU
...
async def finish_button_visibility(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await context.bot.send_message(chat_id=user_id, text=text, reply_markup=get_settings_keyboard())
...
# Текстовые лейблы reply-кнопок главного меню. Используется
# `_handle_menu_escape` ниже, чтобы корректно прерывать FSM-состояния, если
# пользователь, находясь в каком-то шаге диалога, нажал кнопку главного меню.
_MAIN_MENU_BUTTONS_RAW = {
    "📅 Сегодня", "📅 Завтра", "📅 Расписание",
    "📝 Домашнее задание",
    "🔔 Звонки", "👨‍🏫 Учителя",
    "🎉 Каникулы", "💬 Написать админу",
...
async def _handle_menu_escape(update, context):
    """Если пользователь, находясь в каком-то FSM-состоянии, прислал текст,
    ...
    if not text:
        return None
    # Учитываем переименованные пользователем кнопки.
    user_id = str(update.effective_user.id) if update.effective_user else None
    user = get_user(user_id) if user_id else None
    rename_map = getattr(user, 'custom_buttons', {}) if user else {}
    candidates = set(_MAIN_MENU_BUTTONS_RAW)
    if rename_map:
...
async def handle_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message_text == "📅 Сегодня":
        ...
        # Используем локальное время пользователя — иначе для пользователей
        # с другой timezone день недели и дата могли определяться неверно.
        local_today_dt = _user_local_now(user)
        today_index = local_today_dt.weekday()
        day_name = get_day_name(today_index)
        schedule = get_day_schedule(class_obj, day_name)

        today_str = local_today_dt.strftime("%Y-%m-%d")
        ...
        tomorrow = _user_local_now(user) + timedelta(days=1)
        ...
        bells_info = get_bells_info(class_obj)
        ...
        holidays_info = get_holidays_count(class_obj)
...
async def precheckout_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith("buy_stars_"):
        ...
        await query.answer(ok=True)
    elif payload.startswith("buy_broadcast_"):
        await query.answer(ok=True)
    else:
...
async def successful_payment_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payload.startswith("buy_stars_"):
        ...
        context.user_data['broadcast_paid'] = True
        warning_text = "⚠️ **ВНИМАНИЕ!**\n\nРассылка будет отправлена ВСЕМ пользователям бота.\nЭто можно сделать максимум 2 раза.\n\nВведите текст сообщения для рассылки:"

        await update.message.reply_text(warning_text, parse_mode=ParseMode.MARKDOWN, reply_markup=get_cancel_keyboard())
...
@timeout(CONVERSATION_TIMEOUT)
async def paid_broadcast_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    stats = load_stars_stats()
    user_broadcasts = sum(1 for tx in stats.get('transactions', []) 
                         if tx.get('user_id') == user_id and 'Рассылка' in tx.get('description', ''))

    if user_broadcasts >= 2:
        await query.edit_message_text(
            "❌ Вы уже использовали максимальное количество рассылок (2)."
        )
        return await stars_menu(update, context)

    price = PRICES['broadcast']

    if user.stars_balance >= price:
        add_stars_transaction(user_id, -price, f"Рассылка всем пользователям")
        context.user_data['broadcast_paid'] = True

        warning_text = "⚠️ **ВНИМАНИЕ!**\n\nРассылка будет отправлена ВСЕМ пользователям бота.\nЭто можно сделать максимум 2 раза.\n\nВведите текст сообщения для рассылки:"

        await query.edit_message_text(warning_text, parse_mode=ParseMode.MARKDOWN, reply_markup=get_cancel_keyboard())
        return ENTER_BROADCAST_MESSAGE
    else:
        prices = [LabeledPrice(label="Рассылка", amount=price)]

        await context.bot.send_invoice(
            chat_id=user_id,
            title="Рассылка",
            description="Отправить сообщение всем пользователям",
            payload=f"buy_broadcast_{price}",
            provider_token="",
            currency="XTR",
            prices=prices,
...
async def send_broadcast_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.user_data.get('broadcast_paid'):
        await update.message.reply_text("❌ Рассылка не оплачена.")
        return await stars_menu(update, context)

    rejected = await reject_if_forbidden_chars(update, message_text, PURCHASE_BROADCAST)
    if rejected is not None:
        return rejected
    ...
    for uid, u in users.items():
        if not u.is_blocked and uid != user_id:
            try:
                await context.bot.send_message(
                    chat_id=uid,
                    text=f"📢 **Рассылка от {user.first_name}:**\n\n{message_text}",
                    parse_mode=ParseMode.MARKDOWN
                )
                sent_count += 1
            except Exception as e:
    ...
    context.user_data.pop('broadcast_paid', None)
    return await stars_menu(update, context)
...
async def _notify_developer_about_new_user(context, new_user):
    """Шлёт разработчику уведомление о появлении нового пользователя.
    Уважает тоггл `notify_new_users` в настройках разработчика — если он
    его отключил, ничего не делаем. Не падает, если разработчик не задан
    или его аккаунт ещё не создан в боте.
    """
    if not DEVELOPER_ID:
        return
    if str(new_user.user_id) == str(DEVELOPER_ID):
        # Сам разработчик — не уведомляем сам себя.
        return
    dev_user = get_user(DEVELOPER_ID)
    if dev_user is not None and not getattr(dev_user, 'notify_new_users', True):
        return
    ...
    text = (
        ...
        f"📅 Зарегистрирован: {new_user.joined_date}"
    )
    try:
        await context.bot.send_message(chat_id=int(DEVELOPER_ID), text=text)
    except Exception as e:
        logger.warning(f"send dev new-user notification failed: {e}")
...
async def dev_toggle_new_user_notif(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Включает/выключает у разработчика приём уведомлений о новых
    пользователях. Доступно только пользователю с user_id == DEVELOPER_ID.
    """
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    if user_id != DEVELOPER_ID:
        ...
        return MAIN_MENU
    user = get_user(user_id) or User(user_id)
    user.notify_new_users = not getattr(user, 'notify_new_users', True)
    save_user(user)
    status = "включены" if user.notify_new_users else "выключены"
    try:
        await query.edit_message_text(
            f"✅ Уведомления о новых пользователях {status}.",
        ...
        try:
            await context.bot.send_message(
                chat_id=int(user_id),
                text=f"✅ Уведомления о новых пользователях {status}.",
                reply_markup=get_developer_keyboard(),
            )
        except Exception:
            pass
...
async def batch_add_teachers_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Запускает пакетное добавление учителей: админ присылает одно сообщение,
    в каждой строке — «Предмет: Учитель» (или «Предмет - Учитель»). Можно за
    раз добавить хоть весь список. Если предмет уже есть — учитель просто
    обновится."""
    query = update.callback_query
    await query.answer()
    text = (
        "📥 *Добавить всех сразу*\n\n"
        "Отправь одним сообщением список вида:\n"
        "`Русский: Иванова Анна Петровна`\n"
...
@timeout(CONVERSATION_TIMEOUT)
async def batch_add_teachers_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Парсит многострочный ввод учителей и сохраняет всё разом."""
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    if not user:
        user = User(user_id)
    ...
    rejected = await reject_if_forbidden_chars(update, raw, BATCH_ADD_TEACHERS)
    if rejected is not None:
        return rejected
    ...
    for raw_line in raw.split("\n"):
        ...
        if ":" in line:
            ...
            continue
        subj = subj.strip()
        teacher = teacher.strip() or "Не указан"
        if not subj:
            skipped.append(line)
            continue
...
async def add_homework_date_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('adding_new_hw_subject'):
        ...
        if not class_obj:
            await update.message.reply_text("❌ Класс не найден.")
            return await admin_panel(update, context)
        # Синхронизируем subjects/teachers — чтобы один и тот же предмет
        # автоматически появился и в списке учителей (без обязанности
        # вписывать ФИО — там подставится «Не указан»). Иначе админ должен
        # был бы вбивать предмет дважды: для ДЗ и для учителей.
        changed = False
        if text_in not in class_obj.subjects:
            class_obj.subjects.append(text_in)
            changed = True
        if text_in not in class_obj.teachers:
            class_obj.teachers[text_in] = "Не указан"
            changed = True
        if changed:
            classes = load_classes()
            classes[class_code] = class_obj
...
def _next_date_for_weekday(today, target_dow):
    """Возвращает ближайшую дату (>= today), у которой weekday() == target_dow.
    target_dow: 0..6 (понедельник..воскресенье)."""
    if not (0 <= target_dow <= 6):
        return today
    diff = (target_dow - today.weekday()) % 7
    return today + timedelta(days=diff)


async def _ask_homework_date(update, context, subject):
    """ПУНКТ 9: показывает быстрые кнопки выбора даты вместо ручного ввода.
    Помимо «Сегодня/Завтра/+2/+7» админ может выбрать любой день недели
    (Пн-Вс) — ближайшую такую дату подставляем автоматически."""
    # Дату строим в локальном времени админа, иначе при разных часовых
    # поясах «сегодня» может определяться неверно.
    user_id = str(update.effective_user.id) if update.effective_user else None
    user = get_user(user_id) if user_id else None
    today = (_user_local_now(user) if user else datetime.utcnow() + timedelta(hours=3)).date()
    keyboard = [
        [
            InlineKeyboardButton(f"📅 Сегодня ({today.strftime('%d.%m')})", callback_data=f"hw_date_{today.strftime('%Y-%m-%d')}"),
            InlineKeyboardButton(f"📅 Завтра ({(today + timedelta(days=1)).strftime('%d.%m')})", callback_data=f"hw_date_{(today + timedelta(days=1)).strftime('%Y-%m-%d')}"),
        ],
        [
            InlineKeyboardButton(f"📅 +2 дня ({(today + timedelta(days=2)).strftime('%d.%m')})", callback_data=f"hw_date_{(today + timedelta(days=2)).strftime('%Y-%m-%d')}"),
            InlineKeyboardButton(f"📅 +7 дней ({(today + timedelta(days=7)).strftime('%d.%m')})", callback_data=f"hw_date_{(today + timedelta(days=7)).strftime('%Y-%m-%d')}"),
        ],
    ]
    # День недели — две строки по 4 и 3 кнопки. Считаем для каждого ближайшую
    # дату (не раньше «сегодня»), чтобы админу не приходилось думать про
    # реальную дату вручную.
    dows = [
        ("Пн", 0), ("Вт", 1), ("Ср", 2), ("Чт", 3),
    ]
    keyboard.append([
        InlineKeyboardButton(
            f"{label} ({_next_date_for_weekday(today, dow).strftime('%d.%m')})",
            callback_data=f"hw_dow_{dow}",
        )
        for label, dow in dows
    ])
    dows2 = [
        ("Пт", 4), ("Сб", 5), ("Вс", 6),
    ]
...
async def hw_date_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ПУНКТ 9: обработчик кнопок даты (Сегодня/Завтра/+2/+7/custom/Пн-Вс)."""
    query = update.callback_query
    await query.answer()
    if query.data == "hw_date_custom":
        ...
        return ADD_HOMEWORK
    if query.data.startswith("hw_dow_"):
        try:
            target_dow = int(query.data.replace("hw_dow_", ""))
        except ValueError:
            target_dow = 0
        user_id = str(query.from_user.id)
        user = get_user(user_id)
        today = (_user_local_now(user) if user else datetime.utcnow() + timedelta(hours=3)).date()
        date_obj = _next_date_for_weekday(today, target_dow)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "paid_broadcast":
        return await paid_broadcast_handler(update, context)
    ...
    elif data == "dev_toggle_new_user_notif":
        return await dev_toggle_new_user_notif(update, context)
    ...
    elif data == "batch_add_teachers":
        return await batch_add_teachers_start(update, context)
...
async def _self_ping_job(context: ContextTypes.DEFAULT_TYPE):
    """Периодически пингует HTTP-сервер, чтобы бот не засыпал."""
    try:
...
async def enter_city_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Получение города при первичной регистрации (после ввода времени)."""
    user_id = str(update.effective_user.id)
    user = get_user(user_id)
    if not user:
        user = User(user_id)
    ...
    ok, canonical = await weather_validate_city(city_input)
    if not ok:
        await update.message.reply_text(
    ...
    await update.message.reply_text(
        ...
        f"Время и включение можно изменить в ⚙️ Настройки → 🌦 Настройки погоды."
    )
    await class_management(update, context)
    return CLASS_MANAGEMENT
...
async def weather_settings_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"🌦 **Настройки погоды**\n\n"
        f"Утренние уведомления: {'Включены' if enabled else 'Выключены'}\n"
        f"⏰ Время уведомления: {weather_time}\n"
        f"🏙 Город: {city}\n\n"
        f"Выберите действие:"
    )
    try:
...
async def change_city_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Не удалось перепланировать погоду после смены города: {e}")
    await update.message.reply_text(
...
async def _send_user_holiday_notification(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not getattr(user, 'holiday_notifications', True):
        return
    # Дата считается в часовом поясе пользователя — иначе уведомление
    # о празднике могло прилететь не в тот день.
    today_key = _user_local_now(user).strftime("%d-%m")
    holidays = load_holidays()
...
#  каждого утра/вечера/погоды/праздника/ДР. На некоторых хостингах эти
#  cron-джобы молча не срабатывали (битая tzdata, нестабильный APScheduler) —
#  пользователь не получал ни утренние, ни вечерние, ни ДР, ни праздничные
#  уведомления. Так же терялись таймеры.
#
...
#  надёжный, не зависящий от системного tzdata, стабильно работает на
#  Render/Railway/любом VPS.
#
...
def _should_fire_notif(local_now, target_hhmm, window_minutes=120):
    """Возвращает True, если локальное время пользователя находится в окне
    [target, target + window_minutes) текущего дня. Используется в едином
    тикере уведомлений вместо строгого `local_hhmm == target` — иначе при
    дрожании планировщика тикер мог «перепрыгнуть» нужную минуту, и
    уведомление пропускалось на весь день.

    Окно по умолчанию **120 минут** — это компромисс: на бесплатных хостингах
    (Render free, Fly free, Heroku eco) сервис засыпает после 15 минут без
    HTTP-трафика, а холодный старт занимает 30–60 секунд. Если бот ушёл спать
    в 07:50 и проснулся только когда кто-то пингнул в 08:30, узкое окно (5–30
    мин) пропустит «доброе утро» совсем. С окном 2 часа уведомление всё равно
    придёт — пусть с опозданием, но придёт. Дублей не будет: каждое
    уведомление помечается в `notification_log` как отправленное на этот день.
    """
    if not target_hhmm:
        return False
    try:
        h_str, m_str = target_hhmm.split(':')
        target_minutes = int(h_str) * 60 + int(m_str)
...
async def _tick_send_birthday_for_user(bot, user, log, local_today_str, context=None):
    """Отправляет уведомление «через сколько дней мой ДР» / поздравление в сам
    день рождения. Соблюдает birthday_personal_notification и
    show_birthday_to_class — в день рождения, если пользователь не запретил
    показ классу, рассылает уведомление одноклассникам."""
    ...
    try:
        ...
        # Личное уведомление имениннику (или отсчёт). Уважаем флаг
        # birthday_personal_notification — если он выключен, личное
        # уведомление пропускаем, но классную рассылку всё равно делаем,
        # если пользователь её не запрещал.
        if getattr(user, 'birthday_personal_notification', True):
            if is_birthday_today:
                ...
                days_left = _days_until_birthday_for_user(user)
                if days_left is not None and days_left > 0 and getattr(user, 'show_birthday_countdown', True):
                    word = _format_days_word(days_left)
                    message = (
                        f"🎂 До твоего дня рождения осталось {days_left} {word}!\n"
                        f"📅 Дата: {user.birthday}"
        ...
        # Уведомление классу — только в сам день рождения и если пользователь
        # это разрешил. Шлём всем одноклассникам (включая админов), кроме
        # самого именинника.
        if is_birthday_today and getattr(user, 'show_birthday_to_class', True):
            try:
                class_obj = get_class_by_user(user_id)
                if class_obj:
                    name = user.first_name or f"@{user.username}" if user.username else f"User {user_id}"
                    class_msg = (
                        f"🎂 Сегодня день рождения у одноклассника {name}!\n"
                        f"Поздравьте его(её) с праздником! 🎉"
                    )
                    recipients = set(class_obj.students or [])
                    recipients.update(class_obj.admins or [])
                    blocked = set(class_obj.blocked_users or [])
                    for member_id in recipients:
                        if str(member_id) == str(user_id):
                            continue
                        if str(member_id) in blocked:
                            continue
                        try:
                            await bot.send_message(chat_id=int(member_id), text=class_msg)
                        except Exception as e:
                            logger.warning(f"tick/birthday: class notify to {member_id} failed: {e}")
            except Exception as e:
                logger.error(f"tick/birthday: class notify error for {user_id}: {e}")
...
async def _unified_notification_tick(context: ContextTypes.DEFAULT_TYPE):
    ...
    Не зависит от cron-trigger в APScheduler — поэтому надёжно работает
    в любых окружениях, в т.ч. с битым tzdata.
    """
    bot = context.bot
    # 1) Таймеры
    try:
    ...
    for uid, user in users.items():
        try:
            ...
            if getattr(user, 'notifications', True):
                ...
                if _should_fire_notif(local_now, morning_t) and not _notification_already_sent(
                    log, uid, 'morning', local_today_str
                ):
                    try:
                        await bot.send_message(
                            chat_id=int(uid),
            ...
            if getattr(user, 'notifications', True):
                ...
                if _should_fire_notif(local_now, evening_t) and not _notification_already_sent(
                    log, uid, 'evening', local_today_str
                ):
                    try:
                        await bot.send_message(
                            chat_id=int(uid),
            ...
            if getattr(user, 'weather_notifications', True) and getattr(user, 'city', None):
                ...
                if _should_fire_notif(local_now, weather_t) and not _notification_already_sent(
                    log, uid, 'weather', local_today_str
                ):
                    try:
                        text = await weather_current_text(user.city)
                        ...
                        log_changed = True
                        logger.info(f"unified_tick: погода -> {uid} отправлено.")
                    except Exception as e:
                        logger.error(f"unified_tick: weather {uid}: {e}")

            # 2d) Праздники (привязаны к тому же времени, что и погода)
            if getattr(user, 'holiday_notifications', True):
                holiday_t = getattr(user, 'weather_notification_time', '07:00') or '07:00'
                if _should_fire_notif(local_now, holiday_t) and not _notification_already_sent(
                    log, uid, 'holiday', local_today_str
                ):
                    try:
            ...
            if _should_fire_notif(local_now, bday_t) and not _notification_already_sent(
                log, uid, 'birthday', local_today_str
            ):
                await _tick_send_birthday_for_user(bot, user, log, local_today_str, context=context)
                log_changed = True
        except Exception as e:
...
async def _post_init(application):
    ...
    HTTP-сервер запускается отдельной задачей в asyncio.gather (см. main_async),
    поэтому здесь его поднимать не нужно."""
    try:
...
def build_application():
    """Собирает Application с хендлерами и возвращает его. Запуск (поллинг)
    делается отдельно в main_async() через asyncio.gather, чтобы параллельно
    с ботом мог работать HTTP-сервер."""
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            BATCH_ADD_TEACHERS: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, batch_add_teachers_handler),
                CallbackQueryHandler(handle_callback),
            ],
            EDIT_BELLS: [
                CallbackQueryHandler(handle_callback),
            ...
            ENTER_BROADCAST_MESSAGE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, send_broadcast_handler),
                CallbackQueryHandler(handle_callback),
            ],
...
async def main_async():
    """Асинхронный entry-point: инициализирует MongoDB, прогревает кэш
    пользователей и параллельно запускает бот (polling) и HTTP-сервер."""
    # 1. MongoDB Atlas
    await init_mongo()
    await bulk_load_users_from_mongo()

    # 2. Сборка Application и его post_init/handlers
    application = build_application()

    # 3. Параллельный запуск: бот (polling) + HTTP-сервер для Render.
    try:
