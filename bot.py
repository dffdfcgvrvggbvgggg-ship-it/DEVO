...
def _should_fire_notif(local_now, target_hhmm, window_minutes=30):
    ...
    уведомление пропускалось на весь день. Окно по умолчанию 30 минут —
    достаточно, чтобы поймать любые реальные задержки/рестарты, но не
    настолько широкое, чтобы доброе утро прилетело вечером.
    """
    if not target_hhmm:
        return False
    try:
