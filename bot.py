...
async def auto_reenter_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not getattr(update, "message", None):
        return MAIN_MENU
    # ПУНКТ 12 (раунд 4): если у пользователя стоит флаг "replying_to_anon",
    # то ЛЮБОЙ его текстовый ответ — это ответ на анонимное сообщение,
    # даже если ConversationHandler «потерял» состояние. Без этой ветки
    # анонимный ответ молча проглатывался автореинтером и не доходил до
    # send_anonymous_reply.
    if context.user_data.get('replying_to_anon'):
        logger.info("auto_reenter_main_menu: replying_to_anon=True → send_anonymous_reply")
        try:
            return await send_anonymous_reply(update, context)
        except Exception as e:
            logger.error(f"auto_reenter_main_menu→send_anonymous_reply error: {e}")
            try:
                await update.message.reply_text("Не удалось отправить ответ. Попробуйте ещё раз.")
            except Exception:
                pass
...
async def anon_pay_virt_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    user_id = str(query.from_user.id)
    logger.info(f"anon_pay_virt_handler: user_id={user_id} data={query.data}")
    user = get_user(user_id)
    if not user:
        user = User(user_id)
...
async def anon_pay_xtr_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    user_id = str(query.from_user.id)
    logger.info(f"anon_pay_xtr_handler: user_id={user_id} data={query.data}")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПУНКТ 12 (раунд 4): анонимные сообщения должны работать ВСЕГДА —
    # получатель не должен утыкаться в стенку «подпишитесь на канал», если
    # он уже взаимодействовал с ботом и ему пришло анонимное сообщение.
    # Поэтому диспатчим эти колбэки ДО проверки подписки.
    anon_callback_prefixes = (
        "view_anon_msg_",
        "reply_anon_",
        "pay_view_sender_",
        "anon_pay_virt_",
    ...
    anon_callback_exact = {"view_anon_list", "cancel_anon_reply", "cancel_anon"}
    is_anon_callback = (
        data in anon_callback_exact
        or any(data.startswith(p) for p in anon_callback_prefixes)
    )
    if is_anon_callback:
        logger.info(f"handle_callback: anon callback data={data}")
        try:
            ...
            logger.error(f"anon callback {data} crashed: {e}", exc_info=True)
            try:
                await query.edit_message_text(
...
async def start_anon_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Используем split с лимитом 2, чтобы корректно обрабатывать id с подчёркиваниями
    # (на всякий случай — текущие id это [a-z0-9]{12}).
    parts = query.data.split("_", 2)
    msg_id = parts[2] if len(parts) >= 3 else ""
    user_id = str(query.from_user.id)
    logger.info(f"start_anon_reply: user_id={user_id} msg_id={msg_id} data={query.data}")
    ...
    try:
        ...
        logger.error(f"start_anon_reply edit_message_text error: {e}")
        try:
            # Если редактирование сломалось (старое сообщение или иной чат) —
            # просто отправим новое, чтобы пользователь увидел подсказку.
            await context.bot.send_message(
                chat_id=user_id,
                text="💬 Введите ваш ответ на анонимное сообщение:",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Отмена", callback_data="cancel_anon_reply")]]),
            )
        except Exception:
            pass
...
async def send_anonymous_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(
        f"send_anonymous_reply: from={user_id} to={to_user_id} "
        f"len={len(reply_text or '')}"
    )
    ...
    try:
        ...
        logger.info(f"send_anonymous_reply: OK from={user_id} to={to_user_id}")
    except Exception as e:
        # Распространённая причина: получатель ещё не начал чат с ботом или
        # заблокировал его. Сообщим об этом явно, чтобы пользователь понимал.
        logger.error(f"send_anonymous_reply: send to {to_user_id} failed: {e}")
        await update.message.reply_text(
            "Не удалось отправить ответ. Возможно, получатель ещё не начал\n"
            "чат с ботом или заблокировал его."
        )
...
def _user_local_now(user):
    ...
    try:
        ...
        s = str(tz_raw or "").strip().upper().replace("UTC", "").replace(" ", "")
        try:
            tz_int = int(s) if s else 3
        except Exception:
            tz_int = 3
    return _now_utc() + timedelta(hours=tz_int)
...
def main():
    ...
    # ПУНКТ 12 (раунд 4): глобальные обработчики анонимных сообщений.
    # Если ConversationHandler по какой-то причине не подхватит этот колбэк
    # (например, состояние пользователя слетело, или это входящий DM от
    # бота к юзеру, который никогда не нажимал /start), эти обработчики
    # сработают как страховка. В обычной работе они «молчат», потому что
    # conv_handler.add_handler стоит выше и перехватывает первый.
    application.add_handler(
        CallbackQueryHandler(view_anon_message_detail, pattern=r"^view_anon_msg_"),
        group=1,
    )
    application.add_handler(
        CallbackQueryHandler(view_anonymous_messages, pattern=r"^view_anon_list$"),
        group=1,
    )
    application.add_handler(
        CallbackQueryHandler(start_anon_reply, pattern=r"^reply_anon_"),
        group=1,
    )
    application.add_handler(
        CallbackQueryHandler(pay_view_sender_handler, pattern=r"^pay_view_sender_"),
        group=1,
    )
    application.add_handler(
        CallbackQueryHandler(anon_pay_virt_handler, pattern=r"^anon_pay_virt_"),
        group=1,
    )
    application.add_handler(
        CallbackQueryHandler(anon_pay_xtr_handler, pattern=r"^anon_pay_xtr_"),
        group=1,
    )
    application.add_handler(
        CallbackQueryHandler(cancel_anon_reply_handler, pattern=r"^cancel_anon_reply$"),
        group=1,
    )

    # Глобальный MessageHandler в группе 1: ловит текстовый ответ, если
    # у пользователя стоит флаг replying_to_anon, на случай если conv_handler
    # потерял состояние (например, после рестарта или таймаута).
    async def _global_anon_reply_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        if not getattr(update, "message", None):
            return
        if not ctx.user_data.get('replying_to_anon'):
            return
        logger.info("_global_anon_reply_text: catching anon reply outside conv")
        try:
            await send_anonymous_reply(update, ctx)
        except Exception as e:
            logger.error(f"_global_anon_reply_text crashed: {e}")

    application.add_handler(
        MessageHandler(filters.TEXT & ~filters.COMMAND, _global_anon_reply_text),
        group=1,
    )
